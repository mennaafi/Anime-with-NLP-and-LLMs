...
class CharacterNetworkGenerator():
    ...
    def generate_character_network(self,df):
        ...
        relationship_df['character_1'] = relationship_df['value'].apply(lambda x: x[0])
        relationship_df['character_2'] = relationship_df['value'].apply(lambda x: x[1])
        relationship_df = relationship_df.groupby(['character_1', 'character_2']).count().reset_index()
